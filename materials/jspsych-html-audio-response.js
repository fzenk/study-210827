...
jsPsych.plugins["html-audio-response"] = (function() {
    ...
    plugin.info = {
        ...
        parameters: {
            ...
            countdown: {
                type: jsPsych.plugins.parameterType.STRING,
                pretty_name: 'Show countdown while recording',
                default: '<span id="countdown" style="visibility:visible;"><span id="clock" style="color:darkorange; font-weight:bold;">1:00</span></span>',
                description: 'Countdown until end of recording.'
            },
            retry_message: {
                type: jsPsych.plugins.parameterType.STRING,
                pretty_name: 'Retry Message',
                default: 'Retry',
                description: 'Text for button to record response again'
            },
            accept_message: {
                type: jsPsych.plugins.parameterType.STRING,
                pretty_name: 'Accept Message',
                default: 'Accept Recording',
                description: 'Text for button to accept recording and continue to next trial'
            }
        }
    };
    ...
    plugin.trial = function(display_element, trial) {
        ...
        html += '<div id="jspsych-html-audio-response-recording-container">'+trial.recording_light_off+
        '</div>';
        ...
        html += '<div id="jspsych-html-audio-response-buttons"><button id="jspsych-html-audio-response-rerecord" class="jspsych-audio-response-button jspsych-btn" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'; visibility:hidden;">'+trial.retry_message+'</button><button id="jspsych-html-audio-response-okay" class="jspsych-audio-response-button jspsych-btn" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'; visibility:hidden;">'+trial.accept_message+'</button></div>';
        ...
        function start_trial() {
            ...
            document.querySelector('#jspsych-html-audio-response-recording-container').innerHTML = trial.recording_light+' '+trial.countdown;
            // trial start time
            start_time = performance.now();
            // show countdown
            wait_time = trial.buffer_length;
            beginning = performance.now();
            interval = setInterval(function() {
                time_left = wait_time - (performance.now() - beginning);
                minutes = Math.floor(time_left / 60000);
                seconds = Math.floor((time_left - minutes * 60000) / 1000);
                seconds_str = seconds.toString().padStart(2,'0');
        ...
        function start_recording() {
            ...
            if (!trial.wait_for_mic_approval) {
                ...
                display_element.querySelector('#countdown').style.visibility = 'visible';
            }
        }
